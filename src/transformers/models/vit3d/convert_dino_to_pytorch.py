...
"""Convert Vit3d checkpoints trained with the DINO method."""
...
from transformers import Vit3dConfig, Vit3dForImageClassification, Vit3dImageProcessor, Vit3dModel
...
@torch.no_grad()
def convert_vit3d_checkpoint(model_name, pytorch_dump_folder_path, base_model=True):
    """
    ...
    if model_name in ["dino_vit3ds8", "dino_vit3ds16"]:
    ...
    if base_model:
        model = Vit3dModel(config, add_pooling_layer=False).eval()
    else:
        model = Vit3dForImageClassification(config).eval()
    model.load_state_dict(state_dict)

    # Check outputs on an image, prepared by Vit3dImageProcessor
    image_processor = Vit3dImageProcessor()
...
if __name__ == "__main__":
    ...
    convert_vit3d_checkpoint(args.model_name, args.pytorch_dump_folder_path, args.base_model)
